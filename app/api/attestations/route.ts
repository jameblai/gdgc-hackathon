  ...
  try {
    ...
    if (!claim_id || !user_id || !type || graph_distance === undefined) {
      return NextResponse.json(
        {
          error:
            "Missing required fields: claim_id, user_id, type, graph_distance",
        },
        { status: 400 },
      );
    }
    ...
    if (claimRow.userId === user_id) {
      return NextResponse.json(
        { error: "Cannot attest your own claim" },
        { status: 400 },
      );
    }
    ...
    const domainRow = claimRow.domain
      ? await db
          ...
          .where(
            and(
              eq(userDomains.userId, user_id),
              eq(userDomains.domain, claimRow.domain),
            ),
          )
          .limit(1)
      : [];
    ...
    const newBaseTrust = isNeutral
      ? attestantRow.baseTrust
      : ema(attestantRow.baseTrust, isPositive);
    const newAttestAccuracy = isNeutral
      ? attestantRow.attestationAccuracyScore
      : ema(attestantRow.attestationAccuracyScore, isPositive);
    const newParticipation = ema(attestantRow.participationScore, true); // participation always nudges up on any action
    const newDomainTrust = isNeutral
      ? currentDomainTrust
      : ema(currentDomainTrust, isPositive);
    ...
    if (err?.code === "23505") {
      return NextResponse.json(
        { error: "Already attested this claim" },
        { status: 409 },
      );
    }
