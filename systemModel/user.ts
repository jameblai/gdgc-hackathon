...
export class UserNode {
    ...
    private updateAttestationAccuracyScore(type: ResultProp) {
        ...
        if (type === "positive") {
            this.attestationAccuracyScore += alpha * (1 - this.attestationAccuracyScore)
        } else {
...
export class AttestationNode {
    ...
    private calculateWeight(): number {
        const domainTrust =
            this.attestant.domainTrust[this.claim.domain] ?? 0.5

        return (
            this.attestant.baseTrust *
            domainTrust *
            (1 / (1 + this.graphDistance))
        )
    }
}
